...
public class CommunityActivity extends Activity {
	...
	public static CommunityActivity   communityActivity=null;
...
*/  private ImageButton  communityRetBtn;
	...
	private Integer myCommunityImageIconID=R.drawable.human_online_icon/*community_human_online_icon*/;
  ...
  //  	communityView.setVisibility(View.INVISIBLE);
	...
	public void  communityGalleryViewShow(int online_total_num)
	{
		...
		MyLog.printf(TAG,"communityGalleryViewShow000");
			...
			while (it.hasNext())
			{
				...
				for (int i=0;i<community_online_total_num;i++)
				{
					if (communityAccountIds[i].equals("test"))
					{
			    		...
			    		MyLog.printf(TAG,"communityGalleryViewShow111");
...
/*
 * 		communityGallery.setOnItemLongClickListener(new OnItemLongClickListener() {
		...
		//			WSClientService.getInstance().client.setDataChannelOnlyFlag(true);
					try {
						WSClientService.getInstance().client.sendMessage(communityAccountIds[position],MyRtcSip.DATA_CHANNEL_ONLY,null);
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					
					if (!communityAccountNames[position].equals(new String(WSClientService.getUsrName())))	
					{	
						MyLog.printf(TAG,"..............USRNAME=%s",new String(WSClientService.getUsrName()));
			//			WSClientService.getInstance().startMyRtcActivity(communityAccountIds[position],null);
						WSClientService.getInstance().startIPCameraActivity(communityAccountIds[position],nullMyRtcSip.REMOTE_RTC_ACT_INIT_ASK);
			//			finish();
					}
					new callIpCameraTestThread(communityAccountIds[position]).start();
				}					
				return true;
			}
		});
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				MyLog.printf(TAG,"onItemClick............postion=%d",position);				
					...
					{															
						...
						{				
							byte[] bytes=(new String(WSClientService.getUsrName())).getBytes();							
							try 
							{
	...
	private  OnClickListener btnListener= new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (v.getId()==R.id.community_ret_id)
			{
				   ...
				   if (RUN_ON_PAD)
					   intent.setClass(CommunityActivity.this,PadStandbyActivity.class);		   
				   else
					   intent.setClass(CommunityActivity.this,MainMenuActivity.class);
