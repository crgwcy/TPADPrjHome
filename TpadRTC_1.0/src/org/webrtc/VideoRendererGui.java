...
public class VideoRendererGui implements GLSurfaceView.Renderer {
  ...
  public static boolean localVideoShowScalFlag=false;
  public static void setLocalVideoShowScalFlag(boolean localVideoScalFlag)
  {
	 localVideoShowScalFlag=localVideoScalFlag;
  }
  
  public static boolean getLocalVideoShowScalFlag()
  {
	  return localVideoShowScalFlag;
  }
...
*/
      ...
      for(int i=0;i<yuvImageRenderers.size();i++)
      {   	 
    	  if (yuvImageRenderers.size()==2)
    	  {
    		//  MyLog.printf(TAG,"yuvImageRenderers i=%d",i);
    		  if(getLocalVideoShowScalFlag()==false)
        		  yuvImageRenderers.get(i).draw();
    		  else
    		  {	  
    			  if (i==0)
    				  yuvImageRenderers.get(1).draw();  
    			  else if(i==1)
    			  	  yuvImageRenderers.get(0).draw();
    		  }	  
    	  }      	  
    	  else
    	  {
